# scripts/viz_simple.py - Construction Project Management Dashboard (Static Version)
import pandas as pd
import numpy as np
...
def create_gauge_chart(value, title, range_max=100, color=COLORS['primary']):
    ...
    fig = go.Figure(go.Indicator(
        ...
        value = value,
        domain = {'x': [0, 1], 'y': [0, 1]},
        title = {'text': title, 'font': {'size': 14, 'color': COLORS['dark']}},
        gauge = {
...
def create_donut_chart(values, names, title, colors=None):
    ...
    fig.update_layout(
        title={'text': title, 'x': 0.5, 'font': {'size': 14, 'color': COLORS['dark']}},
        height=250,
        margin=dict(l=20, r=20, t=40, b=20),
        paper_bgcolor='white',
        font={'color': COLORS['dark'], 'family': 'Segoe UI'},
...
work_status_fig = create_donut_chart(
    status_counts.values, status_counts.index, 
    "Project Work Status",
...
stage_fig = create_donut_chart(
    stage_counts.values, stage_counts.index,
    "Projects by Stage",
    [COLORS['primary'], COLORS['secondary'], COLORS['accent'], COLORS['warning'], COLORS['info']]
)

# 3. Completion Gauge
completion_gauge = create_gauge_chart(avg_completion, "Project Completion %", 100, COLORS['primary'])

# 4. Duration Gauge
avg_duration_used = (df_main['days_used'] / df_main['duration_days'] * 100).mean()
duration_gauge = create_gauge_chart(avg_duration_used, "Utilized Duration %", 100, COLORS['accent'])

# 5. Budget Combination Chart
monthly_data = df_budget_variance.groupby('month').agg({
    'actual_budget': 'sum',
    'planned_budget': 'sum'
}).reset_index()
...
budget_fig = make_subplots(specs=[[{"secondary_y": True}]])
...
budget_fig.update_yaxes(title_text="Budget Amount ($)", secondary_y=False)
budget_fig.update_yaxes(title_text="Variance (%)", secondary_y=True)

budget_fig.update_layout(
    title={'text': 'Budget Variance', 'x': 0.5, 'font': {'size': 14}},
    height=540,
...
resources_agg = df_resources.groupby('resource_type').agg({
...
resources_fig.update_layout(
    title={'text': 'Actual vs Planned Resources', 'x': 0.5, 'font': {'size': 14}},
    barmode='group',
    height=300,
    margin=dict(l=40, r=40, t=40, b=40),
    paper_bgcolor='white',
...
workload_fig.update_layout(
    title={'text': 'Workload Distribution', 'x': 0.5, 'font': {'size': 14}},
    barmode='stack',
    height=300,
    margin=dict(l=40, r=40, t=40, b=40),
    paper_bgcolor='white',
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H5("Project Portfolio Overview", className="card-title"),
                    html.Div([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=work_status_fig, config={'displayModeBar': False})
                ])
        ...
        dbc.Col([
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=completion_gauge, config={'displayModeBar': False})
                ])
            ], className="shadow-sm mb-4"),
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=duration_gauge, config={'displayModeBar': False})
                ])
            ], className="shadow-sm")
        ], width=4),
        ...
        dbc.Col([
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=budget_fig, config={'displayModeBar': False})
                ])
            ], className="shadow-sm")
        ], width=4)
    ], className="mb-4"),
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=resources_fig, config={'displayModeBar': False})
                ])
            ], className="shadow-sm")
        ], width=6),
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Graph(figure=workload_fig, config={'displayModeBar': False})
                ])
            ], className="shadow-sm")
        ], width=6)
    ], className="mb-4"),
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'debug':
        ...
        print("Static dashboard created. Use 'debug' flag to run server.")