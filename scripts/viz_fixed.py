...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dcc.Graph(id='stages-donut-chart', figure=initial_charts[1])
                ])
            ])
        ], md=6)
    ], className="mb-4"),
...
def update_all_charts(selected_project, selected_types, selected_managers, start_date, end_date):
    ...
    try:
        ...
        if selected_project and selected_project != 'all' and selected_project in status_data['project_id'].values:
            completion_value = status_data[status_data['project_id'] == selected_project]['completion_percent'].iloc[0]
        else:
        ...
        # 4. Progress Trend Chart - NOVO 4º GRÁFICO
...
# CALLBACK 3: Cross-filter interaction
@app.callback(
    Output('project-selector', 'value'),
    [Input('status-pie-chart', 'clickData')]
)
def update_project_selector_from_pie(clickData):
    """Allow clicking on pie chart to filter projects"""
    ...
    # Find first project with that status
    matching_projects = data['project_status'][data['project_status']['status'] == clicked_status]
    if not matching_projects.empty:
        return matching_projects.iloc[0]['project_id']
